# Scope of validation for this delivery

The authoring pass verifies that the meta-prompt package is coherent: its role tree and dependency graph resolve, required paths exist, all planned art exports have unique safe destinations, surface mappings are complete, JSON files parse, helper tests pass, and the ZIP is structurally valid.
...
Version 2.0.0 adds role-specific Astra authorship, gpt-image-2-only art requirements, separated evidence/reference inventories, prompt/output hash checks, and negative tests for wrong models, missing provenance, creative post-processing, and false native verification. The HTML board is ordinary functional documentation, not a generated-art deliverable. See PACKAGE_VALIDATION.json for the actual completed revision checks.
