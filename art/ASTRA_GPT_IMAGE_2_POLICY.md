...
3. The selected author writes the final prompt and stores its exact UTF-8 bytes under `brand/prompts/astra/<asset-id>/<job-id>.md` for the legacy route or `brand/prompts/root/<asset-id>/<job-id>.md` for the root route. Record a SHA-256. The instruction package's briefs are not pre-executed author attestations.
