...
This report indexes actual checks and independent reviews for the private engineering and partial-artwork handoff. The full overhaul is not accepted: native depth-three orchestration and the complete mandatory art set remain outstanding. See `findings.json` for every named review finding and its disposition chain, and the generated final report for requirement states.

| Scope | Result and evidence | Limit |
| --- | --- | --- |
| Integrated package | 218 tests pass for current implementation hashes in `reviews/root-art-local-checks.json`; final package integrity is bound by the external receipt. `reviews/final-local-checks.json` preserves the earlier 195-test result | Local contracts and negative cases, not product/host/provider proof |
...
| Website | Current local artwork candidate: 30 PHPUnit tests / 114 assertions and 30 local HTTP/TLS sink checks pass; `reviews/root-art-subscription-http.json`. Final current-art browser review is recorded separately | Old 110-state matrix retains its old scope; no screen-reader application, production SMTP/Apache or deployment claim |
...
| Content sync |11 focused tests and W03 independent repair review, including forged receipt, edit protection, concurrency and rollback failures | Protected local Git authority; no deployment or creative-art approval |
| Campaign | 12 episodes, 24 clips, 6 article outlines, 72 release gates and event/metric consistency reviewed | Drafts and unobserved outcomes; no campaign operation |
| Art/native model | 32 source masters, 42 exports, 21 verified families; 73 exports missing plus a seamless-tile replacement. `root-art-acceptance.json` and W02/W03 reviews bind actual scope | Root image route is user-authorized; backend/model identity unknown. Required native depth-three chain remains absent; no public distribution approval |
| Private archive | Scope review addresses machine paths and raw diagnostics; final exact archive review is external | Private audit only; future public projection needs separate approval |
