# W01-C2-BUILD — Brand language and naming system

You are a **depth-3 leaf** using `gpt-5.6-luna` with `max`. Report to W01-C2. **Do not spawn any descendants, start another client, or change ancestry.** Read AGENTS.md, the relevant parts of specs/STRATEGY.md, and your bounded task packet. A role prompt does not prove your effective runtime model; the parent/root records that from actual metadata.
...
- `brand/copy.json`
- `migration/repository-plan.json`
...
## Required checks
- Naming matches the approved map except documented execution-time conflicts.
- No unsupported win, 24/7 autonomy, universal integration, or legal-clearance claims.
- Audience and developer copy each identify what is actually available.
