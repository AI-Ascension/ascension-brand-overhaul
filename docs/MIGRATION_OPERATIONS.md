...
Exact-byte copies of a plan share a lock under the operator's home directory at `.local/state/ai-ascension-migration/locks/<plan-sha256>.operation-set.lock`, independent of plan and receipt paths. This is a single-account local lock; coordinate operations from other accounts or hosts separately. A definite API refusal is retained as `failed` and may be re-evaluated by a new invocation with the same valid approval and every fresh preflight gate. Ambiguous requests still require reconciliation. Conflicting target names and source/target dependencies are rejected in both plan construction and apply validation.

Snapshot preconditions cannot be silently overridden by map fields. The planner hashes supplied exact snapshot bytes only after checking that they decode to the supplied snapshot; callers supplying only an object receive its canonical JSON digest. A claimed digest must match those bytes. Malformed API inventory rows and duplicate JSON keys are refused, not discarded.
